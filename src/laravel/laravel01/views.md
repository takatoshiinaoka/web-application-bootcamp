...
tweetの入力や編集など複数の画面で必要となるため，共通のコンポーネントとして作成する．
...
「tweet一覧」と「tweet作成」の 2 種類のリンクを作成しておく．
...
- `index.blade.php`（tweet一覧画面）
- `create.blade.php`（tweet作成画面）
- `show.blade.php`（tweet詳細画面）
- `edit.blade.php`（tweet編集画面）
